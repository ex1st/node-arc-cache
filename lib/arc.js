...
Object.defineProperties(ARcache.prototype, {
	...
	'info': {
		...
		value: function(key) {
			if (this.__has(key)) {
				return {
					created: new Date(this._storage[key].c),
					expire: this._storage[key].e ? new Date(this._storage[key].e) : Infinity
				};
			}
		}
	},
	...
	'__trim': {
		...
		value: function() {
			...
			var keys = Object.keys(self._storage)
				.filter(function(key) {
					return self._storage[key] && !self.__expired(self._storage[key], key);
				});
